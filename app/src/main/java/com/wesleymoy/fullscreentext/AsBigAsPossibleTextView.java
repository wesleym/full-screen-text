 ...
 */
public class AsBigAsPossibleTextView extends View {
    private static final String TAG = "AsBigAsPossibleTextView";
    ...
    public AsBigAsPossibleTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(attrs, 0);
    }

    public AsBigAsPossibleTextView(Context context, AttributeSet attrs, int defStyle) {
    ...
    private void init(AttributeSet attrs, int defStyle) {
        ...
        final TypedArray a = getContext().obtainStyledAttributes(
                attrs, R.styleable.AsBigAsPossibleTextView, defStyle, 0);

        mExampleString = a.getString(
                R.styleable.AsBigAsPossibleTextView_exampleString);
        mExampleColor = a.getColor(
                R.styleable.AsBigAsPossibleTextView_exampleColor,
                mExampleColor);
        // Use getDimensionPixelSize or getDimensionPixelOffset when dealing with
        // values that should fall on pixel boundaries.
        mExampleDimension = a.getDimension(
                R.styleable.AsBigAsPossibleTextView_exampleDimension,
                mExampleDimension);

        if (a.hasValue(R.styleable.AsBigAsPossibleTextView_exampleDrawable)) {
            mExampleDrawable = a.getDrawable(
                    R.styleable.AsBigAsPossibleTextView_exampleDrawable);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        Paint.FontMetrics fontMetrics = mTextPaint.getFontMetrics();
        float size = (-fontMetrics.ascent + fontMetrics.descent) * lines.size() + fontMetrics.leading * (lines.size() - 1);
        float top = paddingTop + (contentHeight - size) / 2;
